...
ext_int0:                                                  ; Interrupt Service Routine (ISR) for INT0
            sbis    pinOutput,    bitOutput                ; is LED in state ON?
            jmp     led_on                                 ; no => jump to 'set LED ON'
led_off:
            cbi     prtOutput,    bitOutput                ; set LED to OFF
            jmp     ext_int0_end                           ; leave Interrupt Service Routine (ISR)
led_on:
            sbi     prtOutput,    bitOutput                ; set LED to ON
